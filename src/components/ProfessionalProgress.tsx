...
const ProfessionalProgress: React.FC<Props> = ({ percent, maxWidth = '100%' }) => {
  const pct = Math.max(0, Math.min(100, Math.round(percent)));
  return (
    <div
      style={{ width: maxWidth, position: 'relative', padding: '4px 0' }}
      role="progressbar"
      aria-valuemin={0}
      aria-valuemax={100}
      aria-valuenow={pct}
    ...
    >
      ...
      <style>{`
        ...
        .tech-progress-cells {
          ...
          background-image: linear-gradient(90deg, rgba(255,255,255,0.15) 50%, transparent 50%);
